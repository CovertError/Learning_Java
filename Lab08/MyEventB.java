...
/**
 * GUI JFrame called MyEventB that contains labels, text fields and buttons
...
public class MyEventB extends JFrame implements ActionListener 
{
    ...
    */
    ...
    {
      //return command string associated with the action
      if(ae.getActionCommand().equals("+")) //if user pressed "+" button, add 1 to the displayed value
      {     
         counter++;
         jtfValue.setText("" + counter);
      }
      
      else if (ae.getActionCommand().equals("-"))//if user pressed "-" button, subtract 1 from the displayed value
      {
         counter--;
         jtfValue.setText("" + counter);   
      }
      
      else if(ae.getActionCommand().equals("Reset"))//if user pressed "Reset" button, reset the displayed value to 0
      {
         counter = 0;
         jtfValue.setText("" + counter);   
      }
      
      else if (ae.getActionCommand().equals("Quit")) //if user pressed "Quit" button, halt/exit the program
      {
         System.exit(0);   
      }
    }
